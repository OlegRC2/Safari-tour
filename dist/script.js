...
function form(formSelector, modalSelector, modalContentSelector) {
  ...
  form.addEventListener('submit', function (e) {
    // навешиваем на форму обработчик, срабатывающий при подтверждении формы
    e.preventDefault(); // отменяем стандартное поведение браузера (перезагрузку страницы)

    var statusMessageText = document.createElement('div'); // создаем новый элемент на странице для показа его пользователю в котором будет текст
    ...
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["postData"])('./server.php', formData) // отправялем запрос
    .then(function (data) {
      // получаем ответ с сервера, data - это данные с сервера
      statusMessageText.remove(); // удаляем текстовое сообщение об отправке

      if (myMessage == 'none') {
        // если третий аргумент со своим сообщением не передан, то используем стандартное сообщение
...
function modal(modalSelector, modalBtn, content) {
  ...
  modalButton.addEventListener('click', function () {
    return closeModal();
  }); // навешиваем обработчик клика на кнопку закрытия

  modal.addEventListener('click', function (event) {
    // закрытие окна при клике вне окна
    if (event.target === modal) {
      // если место клика входит в родительский элемент окна, но не само окно (у него другой класс)
      closeModal();
    }
  });
}
...
/***/ "./src/js/modules/sizeMapSection.js":
/*!******************************************!*\
  !*** ./src/js/modules/sizeMapSection.js ***!
  \******************************************/
/*! exports provided: default */
...
function sizeMapSection(cloud, firstSection, headerSelector) {
  // функция для изменения высоты главной секции для корректного отображения следующей секции map. Аргументы cloud - блок с картинками облаков, firstSection - вся главная секция, headerSelector - хедер
  function result() {
    var cloudBlock = document.querySelector(cloud),
        // получаем блок с картинками облаков
    promoSection = document.querySelector(firstSection),
        // получаем главную секцию
    header = document.querySelector(headerSelector); // получаем хедер

    var cloudTop = +window.getComputedStyle(cloudBlock).getPropertyValue("top").replace(/\D/g, ''),
        // получаем расстояние на которое смещены облака сверху от хедера, вырезаем "px" и переводим в число
    cloudHeight = +window.getComputedStyle(cloudBlock).getPropertyValue("height").replace('px', ''),
        // получаем высоту блока с облаками, вырезаем "px" и переводим в число
  ...
  window.addEventListener('resize', function () {
    // срабатывает при изменении ширины экрана
    result();
  });
}

/* harmony default export */ __webpack_exports__["default"] = (sizeMapSection);
...
"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _modules_sizeMapSection__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./modules/sizeMapSection */ "./src/js/modules/sizeMapSection.js");
...
window.addEventListener('DOMContentLoaded', function () {
  Object(_modules_sizeMapSection__WEBPACK_IMPORTED_MODULE_0__["default"])('.promo__cloud', '.promo', '.header'); // функция для изменения высоты главной секции для корректного отображения следующей секции "map"

  Object(_modules_form__WEBPACK_IMPORTED_MODULE_1__["default"])('.main-form', '.modal', '.modal__content'); // функция для работы главной формы

  Object(_modules_form__WEBPACK_IMPORTED_MODULE_1__["default"])('.contact-form', '.modal', '.modal__content'); // функция для работы формы для контакта 

  Object(_modules_form__WEBPACK_IMPORTED_MODULE_1__["default"])('.subscription-form', '.modal', '.modal__content', 'Thank you for subscribing!'); // функция для работы формы подписки
